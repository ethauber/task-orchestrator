...
class PingResponse(BaseModel):
    response: str = Field(
        ..., description="Simple echo to verify", examples=["pong", "alive"]
    )
...
class PlanOption(BaseModel):
    ...
    steps: list[PlanStep] = Field(
        ..., description="Ordered list of actionable steps", min_length=3, max_length=7
    )
...
class FinalStep(BaseModel):
    ...
    duration_minutes: int = Field(
        ..., description="Whole minutes. Multiples of 15", examples=[15, 30, 45, 60]
    )
...
class PlanResponse(BaseModel):
    ...
    total_duration: int = Field(
        ..., description="Sum of non-parked durations (minutes).", examples=[90, 180]
    )
...
class PlanSummary(BaseModel):
    """Summary of a saved plan for listing purposes."""
...
class FullPlanResponse(BaseModel):
    """Full details of a saved plan, including metadata."""
    ...
    full_plan_data: PlanResponse = Field(..., description="The complete plan data.")
    created_at: str = Field(..., description="Timestamp when the plan was created.")
    updated_at: str = Field(
        ..., description="Timestamp when the plan was last updated."
    )
